...
spectral_error_df$error = spectral_error_df$spectral_error
EV1_error_df = do.call(rbind, lapply(results, function(x) x$EV1_error))
EV1_error_df$error = EV1_error_df$EV1_error
...
EV1_error_df = left_join(EV1_error_df, truncated %>% mutate(method = NULL) %>% group_by(n, q, replicate, experiment) %>% summarize(truncated = max(truncated)), by = c("n", "q", "replicate", "experiment")) %>%
  mutate(truncated = ifelse(truncated == 0, "not truncated", "truncated"))
...
ggplot(spectral_error_df %>% filter(experiment == "n") %>% mutate(facet = paste0("q = ", q, "; ", estimate)) %>% mutate(facet = factor(facet, levels = gtools::mixedsort(unique(facet)))), aes(x = as.factor(n), y = error, color = method)) +
...
ggsave(file.path(FIGURES_PATH, "simulation_figure_spectral_error_n.pdf"), height = 4, width = 7)
...
ggplot(EV1_error_df %>% filter(experiment == "n") %>% mutate(facet = paste0("q = ", q, "; ", estimate)) %>% mutate(facet = factor(facet, levels = gtools::mixedsort(unique(facet)))), aes(x = as.factor(n), y = error, color = method)) +
  facet_wrap(~facet, scales = "free_y", ncol = 2) +
  geom_boxplot(outlier.size = 0.25) +
  geom_text(data = truncated_proportion %>% filter(experiment == "n") %>% mutate(facet = paste0("q = ", q, "; ", estimate)) %>% mutate(facet = factor(facet, levels = gtools::mixedsort(unique(facet)))), aes(label = paste0(round(proportion * 100, 2), "%"), y = Inf, vjust = 2), size = 2.5) +
  theme_bw() +
  xlab("n") +
  ylab("Eigenvector 1 error") +
  labs(color = "Method") +
  scale_color_manual(values = palette) +
  theme(legend.position = "bottom")
ggsave(file.path(FIGURES_PATH, "simulation_figure_EV1_error_n.pdf"), height = 4, width = 7)

ggplot(EV1_error_df %>% filter(experiment == "n") %>% mutate(facet = paste0("q = ", q, "; ", estimate)) %>% mutate(facet = factor(facet, levels = gtools::mixedsort(unique(facet)))), aes(x = as.factor(n), y = error, color = method)) +
  facet_grid(facet ~ truncated, scales = "free_y") +
  geom_boxplot(outlier.size = 0.25) +
  geom_text(data = EV1_error_df %>% filter(method == "mvHE", experiment == "n") %>% group_by(n, estimate, q, truncated, method) %>% summarize(count = n()) %>% group_by(n, estimate, q, method) %>% mutate(proportion = count / sum(count)) %>% mutate(facet = paste0("q = ", q, "; ", estimate)) %>% mutate(facet = factor(facet, levels = gtools::mixedsort(unique(facet)))), aes(label = paste0(round(proportion * 100, 2), "%"), y = Inf, vjust = 2), size = 2.5) +
  theme_bw() +
  xlab("n") +
  ylab("Eigenvector 1 error") +
  labs(color = "Method") +
  scale_color_manual(values = palette) +
  theme(legend.position = "bottom")
ggsave(file.path(FIGURES_PATH, "simulation_figure_EV1_error_truncated_n.pdf"), height = 8, width = 7)
...
ggplot(spectral_error_df %>% filter(experiment == "q"), aes(x = as.factor(q), y = error, color = method)) +
...
ggplot(EV1_error_df %>% filter(experiment == "q"), aes(x = as.factor(q), y = error, color = method)) +
  facet_wrap(~paste0("n = ", n, "; ", estimate), scales = "free_y", ncol = 2) +
  geom_boxplot(outlier.size = 0.25) +
  geom_text(data = truncated_proportion %>% filter(experiment == "q"), aes(label = paste0(round(proportion * 100, 2), "%"), y = Inf, vjust = 2), size = 2.5) +
  theme_bw() +
  ...
  scale_color_manual(values = palette) +
  theme(legend.position = "bottom")
ggsave(file.path(FIGURES_PATH, "simulation_figure_EV1_error_q.pdf"), height = 4, width = 7)

ggplot(EV1_error_df %>% filter(experiment == "q"), aes(x = as.factor(q), y = error, color = method)) +
  facet_grid(paste0("n = ", n, "; ", estimate) ~ truncated, scales = "free_y") +
  geom_boxplot(outlier.size = 0.25) +
  geom_text(data = EV1_error_df %>% filter(method == "mvHE", experiment == "q") %>% group_by(n, estimate, q, truncated, method) %>% summarize(count = n()) %>% group_by(n, estimate, q, method) %>% mutate(proportion = count / sum(count)), aes(label = paste0(round(proportion * 100, 2), "%"), y = Inf, vjust = 2), size = 2.5) +
  theme_bw() +
  xlab("q") +
  ylab("Eigenvector 1 error") +
  labs(color = "Method") +
  scale_color_manual(values = palette) +
  theme(legend.position = "bottom")
ggsave(file.path(FIGURES_PATH, "simulation_figure_EV1_error_truncated_q.pdf"), height = 8, width = 7)
